...
# This file generates the data for the dependency parser and finally stores the parse trees for testing data
posVocab={}
parseVocab={}
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if line=="":
            ...
            cPickle.dump([id, root, chunk, pos, parentid, parse],open("./Parsed_output_test/" + previd + ".pkl", "wb"))
            break
        else:
    ...
    cPickle.dump(posVocab, open("./Parsed_output_test/posVocab.pkl", "wb"))
    cPickle.dump(parseVocab,open("./Parsed_output_test/parseVocab.pkl", "wb"))
    '''fpdump = open("test.txt", "w")
    for key in test_set:
        print key
        sentIndex, entityIdDict, entityIndexDict, eventIdDict, eventIndexDict = cPickle.load(
            open(processedpathtest + key + ".pkl", 'rb'))
