...
my @target_eclass = ('Gene_expression','Transcription','Development','Growth','Remodeling','Breakdown','Death','Cell_proliferation','Blood_vessel_development',        'Localization','Catabolism','Synthesis','Phosphorylation','Dephosphorylation');
my @target_rclass = ('Regulation', 'Positive_regulation', 'Negative_regulation','Planned_process');
...
foreach my $fname (@ARGV) {
    ...
    ## event loading
    if (!($textlen = &read_text_file("$gdir/PMID-$pmid.txt", $text))) {next}
    if (!&read_a1_file("$gdir/PMID-$pmid.a1", \%protein)) {next}
    if (($num_gold   = &read_a2_file("$gdir/PMID-$pmid.a2", 'G')) < 0) {next}
...
sub eq_args_soft {
    ...
    ##print STDERR "\n$aetype";
    shift @answer_arg;
    ## error coming right here
    my $length_answer=@answer_arg;
    #print STDERR "Arguements:\n";
    #print STDERR join(", ", @answer_arg);
    #print STDERR "$length_answer\n";
    #print STDERR "\nerror coming right here\n";
    #print STDERR "\n@answer_arg";
    while ($answer_arg[-1] !~ /^Theme:/ && $length_answer !~ 0) {pop @answer_arg; $length_answer--;}
    
    my @gold_arg =   @{$gold{$geid}};
    my $getype = shift @gold_arg;
    shift @gold_arg;
    #print STDERR join(", ", @gold_arg);
    my $length_gold=@gold_arg;
    #print STDERR "$length_gold\n";
    while ($gold_arg[-1] !~ /^Theme:/ && $length_gold !~ 0) {pop @gold_arg; $length_gold--;}
