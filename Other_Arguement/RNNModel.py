...
class Relation(object):
	def __init__(self, num_classes, word_dict_size, d1_dict_size, d2_dict_size, type_dict_size,sentMax,wv, w_emb_size=50, d1_emb_size=5, d2_emb_size=5, type_emb_size=5,num_filters=100, l2_reg_lambda = 0.0, pooling='max'):
		...
		self.e= tf.placeholder(tf.int32,[None,None],name="x5")
...
#		W_wemb =    tf.Variable(tf.random_uniform([word_dict_size, w_emb_size], -1.0, +1.0))
		...
		W_gemb= tf.Variable(tf.random_uniform([type_dict_size,type_emb_size],-1.0,+1.0))
		
		# Embedding Layer
		emb0 = tf.nn.embedding_lookup(W_wemb, self.w)				#word embedding
		emb3 = tf.nn.embedding_lookup(W_d1emb, self.d1)				#distance embedding  
		emb4 = tf.nn.embedding_lookup(W_d2emb, self.d2)				#distance embedding  
		emb5 = tf.nn.embedding_lookup(W_gemb,self.e)				#entity type embedding
		...
		h = tf.concat(2, [output_fw, output_bw])
		print 'h', h.get_shape()					
		...
		if pooling == 'max':
		   pooled = tf.nn.max_pool(h, ksize=[1, sentMax, 1, 1], strides=[1, 1, 1, 1], padding='VALID', name="pool")
		else:
		   pooled = tf.reduce_sum(h, 1)
		print 'pooled', pooled.get_shape()
		
		h2 = tf.reshape(pooled, [-1, 2*num_filters])				
		...
		scores = tf.nn.xw_plus_b(h2, W, b, name="scores")			
