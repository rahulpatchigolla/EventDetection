...
#		W_wemb =    tf.Variable(tf.random_uniform([word_dict_size, w_emb_size], -1.0, +1.0))
		...
		emb5 = tf.nn.embedding_lookup(W_typeemb, self.type)			# entity type embedding  
		...
		with tf.variable_scope('rnn'):
			...
			h1_rnn = tf.concat(2, [output_fw, output_bw])				
			print 'h1_rnn', h1_rnn.get_shape()		 
	 		h1_rnn = tf.expand_dims(h1_rnn, -1)						
			...
			h2_rnn = tf.reshape(pooled, [-1, 2*num_filters])				
			print 'h2_rnn', h2_rnn.get_shape()		

		with tf.variable_scope('cnn'):
			# Convolution Layer
			X_cnv = tf.expand_dims(X, -1) 							
